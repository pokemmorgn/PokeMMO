...
NpcDataSchema.statics.createFromJson = async function(
  ...
  console.log(`🔧 [NpcData] Creating NPC from JSON: ${jsonNpc.name} in zone ${zone}`);
  
  // ✅ CORRECTION: Si pas d'ID fourni, générer un ID global unique
  if (!jsonNpc.id) {
    // Trouver le plus grand ID existant dans TOUTES les zones
    const lastNpc = await this.findOne({})
      .sort({ npcId: -1 })
      .select('npcId')
      .lean();
    
    jsonNpc.id = lastNpc ? lastNpc.npcId + 1 : 1;
    console.log(`🔢 [NpcData] Generated global NPC ID: ${jsonNpc.id} (previous max: ${lastNpc?.npcId || 0})`);
  }
  
  // Vérifier unicité globale de l'ID
  const existingNpc = await this.findOne({ npcId: jsonNpc.id });
  if (existingNpc) {
    throw new Error(`NPC ID ${jsonNpc.id} already exists in zone "${existingNpc.zone}"`);
  }
  
  // Créer le NPC avec les données du JSON
  const npcData: any = {
    npcId: jsonNpc.id,
    zone: zone,
    name: jsonNpc.name || `NPC_${jsonNpc.id}`,
    ...
    // Shop (référence simple)
    shopId: jsonNpc.shopId,
    
    // Métadonnées
    isActive: jsonNpc.isActive !== false,
    version: jsonNpc.version || '3.0.0',
    sourceFile: jsonNpc.sourceFile || 'api_import'
  };
  
  // Données spécifiques du type (tout ce qui n'est pas dans les champs de base)
  const baseFields = [
    'id', 'name', 'type', 'position', 'direction', 'sprite',
    'interactionRadius', 'canWalkAway', 'autoFacePlayer', 'repeatable', 'cooldownSeconds',
    'spawnConditions', 'questsToGive', 'questsToEnd', 'questRequirements', 'questDialogueIds',
    'battleConfig', 'visionConfig', 'trainerRuntime', 'shopId',
    'isActive', 'version', 'sourceFile'
  ];
  ...
  console.log(`✅ [NpcData] Created NPC "${newNpc.name}" with global ID ${newNpc.npcId} in zone ${zone}`);
...
NpcDataSchema.statics.bulkImportFromJson = async function(
  ...
  if (!zoneData.npcs || !Array.isArray(zoneData.npcs)) {
    throw new Error('Invalid zone data - npcs array required');
  }
  ...
  // Supprimer les NPCs existants de cette zone (optionnel)
  if (zoneData.clearExisting !== false) {
    const deleteResult = await this.deleteMany({ zone: zoneData.zone });
    console.log(`🗑️ [NpcData] Cleared ${deleteResult.deletedCount} existing NPCs in zone ${zoneData.zone}`);
  }
  ...
  for (const npcJson of zoneData.npcs) {
    try {
      ...
      const errorMsg = `NPC ${npcJson.id || npcJson.name || 'unknown'}: ${error instanceof Error ? error.message : 'Unknown error'}`;
      results.errors.push(errorMsg);
      console.error(`❌ [NpcData] ${errorMsg}`);
    }
  }
  
  console.log(`✅ [NpcData] Bulk import completed: ${results.success} success, ${results.errors.length} errors`);
