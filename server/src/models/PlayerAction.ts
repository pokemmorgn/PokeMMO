...
const PlayerActionSchema = new mongoose.Schema({
  ...
  context: {
    type: {
      ...
      currentMap: { type: String, index: true }, // Pour analyses par map
      zone: { type: String },
      nearbyPlayers: [{ type: String }],
      nearbyNPCs: [{ type: String }],
      isAlone: { type: Boolean, index: true }, // Pour analyses sociales
      friendsOnline: { type: Number },
      currentGold: { type: Number },
      currentLevel: { type: Number },
      teamSize: { type: Number },
      inventoryFull: { type: Boolean },
      recentFailures: { type: Number, index: true }, // Pour détection frustration
      consecutiveActions: { type: Number },
...
PlayerActionSchema.pre('save', function(next) {
  ...
  // Assurer que data et context existent
