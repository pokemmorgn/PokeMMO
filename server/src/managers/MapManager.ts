...
export class MapManager {
    ...
    private extractTeleportsAndSpawns(mapName: string, mapData: TiledMap): void {
            ...
            if (layer.type === 'objectgroup' && layer.objects) {
                for (const obj of layer.objects) {
                    ...
                    if (properties.targetZone && properties.targetSpawn) {
                        ...
                        const targetZone = (properties.targetZone as string).toLowerCase();
                        const targetSpawn = (properties.targetSpawn as string).toLowerCase();

                        if (obj.name?.toLowerCase().includes('spawn')) {
                            // Spawn
                            const spawnKey = `${targetZone}_${targetSpawn}`;
                            ...
                            const teleportKey = `${mapName.toLowerCase()}_teleport_${obj.id}`;
        ...
        for (const [teleportKey, teleport] of this.teleports) {
            if (teleport.mapName === mapName.toLowerCase()) {
    ...
    public getSpawnPoint(mapName: string, spawnName?: string): { x: number; y: number } | null {
        if (spawnName) {
            const spawnKey = `${mapName.toLowerCase()}_${spawnName.toLowerCase()}`;
