...
export class Road1Room extends Room<PokeWorldState> {
  ...
  onCreate(options: any) {
    ...
    this.onMessage("changeZone", async (client, data: { targetZone: string, direction: string }) => {
      ...
      if (player) {
        ...
        console.log(`[Road1Room] Sauvegarde position et map (${spawnX}, ${spawnY}) dans ${data.targetZone} pour ${player.name}`);
      }
  ...
  async saveAllPlayers() {
    try {
      ...
      console.log(`[Road1Room] Sauvegarde automatique : ${this.state.players.size} joueurs`);
    } catch (error) {
      console.error("[Road1Room] Erreur sauvegarde automatique:", error);
    }
  }
