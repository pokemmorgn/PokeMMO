...
async onPlayerJoinZone(client: Client, zoneName: string) {
  ...
  const mappedZoneName = getDbZoneName(zoneName);
  ...
  // ✅ ========== NOUVEAU : ENRICHISSEMENT DES NPCS AVEC CAPACITÉS ==========
  console.log(`🔧 [NPC ENRICHMENT] Début enrichissement ${npcs.length} NPCs...`);
  const enrichedNpcs = [];
  
  for (const npc of npcs) {
    try {
      console.log(`🔍 [NPC ENRICHMENT] Analyse NPC ${npc.id} (${npc.name})...`);
      
      // Utiliser la méthode analyzeNpcCapabilities du NpcInteractionModule
      // On doit passer par une méthode publique ou exposée
      let capabilities = ['dialogue']; // Valeur par défaut
      let contextualData = {
        hasShop: false,
        hasQuests: false,
        hasHealing: false,
      ...
      try {
        // Créer une requête d'interaction fictive pour analyser les capacités
        ...
        // Utiliser le système d'analyse via le module
        const analysisResult = await this.npcInteractionModule.handle(mockContext);
        
        if (analysisResult.success && 'capabilities' in analysisResult) {
          capabilities = (analysisResult as any).capabilities || ['dialogue'];
          contextualData = (analysisResult as any).contextualData || contextualData;
          console.log(`✅ [NPC ENRICHMENT] NPC ${npc.id} analysé avec succès: ${capabilities.join(', ')}`);
        } else {
          console.warn(`⚠️ [NPC ENRICHMENT] Analyse échouée pour NPC ${npc.id}, utilisation valeurs par défaut`);
        }
      } catch (analysisError) {
        console.warn(`⚠️ [NPC ENRICHMENT] Erreur analyse NPC ${npc.id}:`, analysisError);
        
        // Fallback : analyse basique manuelle
        if (npc.questsToGive && Array.isArray(npc.questsToGive) && npc.questsToGive.length > 0) {
          capabilities.push('quest');
          contextualData.hasQuests = true;
          console.log(`🔧 [NPC ENRICHMENT] Fallback: NPC ${npc.id} a des quêtes`);
        }
        
        if (npc.shopId || (npc.properties && npc.properties.shop)) {
          capabilities.push('merchant');
          contextualData.hasShop = true;
          console.log(`🔧 [NPC ENRICHMENT] Fallback: NPC ${npc.id} a une boutique`);
        }
        
        if (npc.type === 'healer' || (npc.properties && npc.properties.healer)) {
          capabilities.push('healer');
          contextualData.hasHealing = true;
          console.log(`🔧 [NPC ENRICHMENT] Fallback: NPC ${npc.id} peut soigner`);
        }
      }
      
      // Créer le NPC enrichi
      const enrichedNpc = {
        ...npc,
      ...
      // Log si le NPC a des capacités intéressantes
      if (capabilities.length > 1 || capabilities.includes('quest')) {
        console.log(`🎯 [NPC ENRICHMENT] NPC ${npc.id} (${npc.name}) enrichi avec capacités:`, capabilities);
        if (capabilities.includes('quest')) {
          console.log(`📜 [NPC ENRICHMENT] NPC ${npc.id} peut donner des quêtes !`);
        }
      }
      
    } catch (enrichmentError) {
      console.error(`❌ [NPC ENRICHMENT] Erreur critique enrichissement NPC ${npc.id}:`, enrichmentError);
      
      // En cas d'erreur critique, utiliser le NPC original avec valeurs par défaut
      enrichedNpcs.push({
        ...npc,
  ...
  console.log(`✅ [NPC ENRICHMENT] Enrichissement terminé: ${enrichedNpcs.length} NPCs enrichis`);
  
  // Compter les NPCs avec capacités spéciales
  const questNpcs = enrichedNpcs.filter(npc => npc.capabilities.includes('quest'));
  const shopNpcs = enrichedNpcs.filter(npc => npc.capabilities.includes('merchant'));
  const healerNpcs = enrichedNpcs.filter(npc => npc.capabilities.includes('healer'));
  
  console.log(`📊 [NPC ENRICHMENT] Résumé capacités:`);
  console.log(`   📜 NPCs avec quêtes: ${questNpcs.length}`);
  console.log(`   🛒 NPCs avec boutique: ${shopNpcs.length}`);
  console.log(`   💊 NPCs soigneurs: ${healerNpcs.length}`);
  
  if (questNpcs.length > 0) {
    console.log(`📜 [NPC ENRICHMENT] NPCs avec quêtes:`, questNpcs.map(npc => `${npc.id}(${npc.name})`));
  }
  // ✅ ========== FIN ENRICHISSEMENT ==========
  
  // ✅ Envoi des NPCs ENRICHIS au client
  try {
    client.send("npcList", enrichedNpcs);  // ← NPCS AVEC CAPACITÉS !
    console.log(`✅ ${enrichedNpcs.length} NPCs ENRICHIS envoyés à ${client.sessionId}`);
    
    // Log spécial pour les NPCs avec quêtes
    questNpcs.forEach(npc => {
      console.log(`🎯 [ENVOI] NPC ${npc.id} (${npc.name}) envoyé avec capacité QUEST !`);
    });
