...
// ENDPOINTS SERVEUR MONGODB À CRÉER
// Ajoutez ces routes à votre serveur Express

const { ObjectId } = require('mongodb');

// 1. GET/POST /mongodb/document - Récupérer un document par ID
app.post('/api/admin/mongodb/document', authenticateToken, async (req, res) => {
    try {
        const { database, collection, id } = req.body;
        ...
        let documentId;
        try {
            // Essayer comme ObjectId d'abord
...
app.post('/api/admin/mongodb/create-document', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/admin/mongodb/update-document', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/admin/mongodb/delete-document', authenticateToken, async (req, res) => {
    try {
        const { database, collection, id } = req.body;
...
app.post('/api/admin/mongodb/query', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/admin/mongodb/aggregate', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/admin/mongodb/database-stats', authenticateToken, async (req, res) => {
    try {
        ...
        for (const collInfo of collections) {
            try {
                ...
                const collStats = await db.collection(collInfo.name).stats();
                ...
                console.warn(`Erreur stats collection ${collInfo.name}:`, error.message);
            }
        }
...
app.get('/api/admin/mongodb/server-info', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({
            ...
            info: {
                ...
                authentication: false, // À adapter selon votre config
                ssl: false // À adapter selon votre config
            }
        });
