...
router.get('/pokemon/:id', requireMacAndDev, async (req: any, res) => {
  try {
    ...
    // Charger d'abord la liste complète (cache potentiel ici en production)
    const pokemonListResponse = await new Promise((resolve, reject) => {
      // Simuler un appel à la route list
      router.handle({
        method: 'GET',
        url: '/pokemon/list',
        user: req.user
    ...
    const pokemon = pokemonListResponse.pokemon.find((p: any) => p.id === pokemonId);
...
router.post('/pokemon/search', requireMacAndDev, async (req: any, res) => {
  try {
    ...
    // Filtrer par nom
    const searchRegex = new RegExp(query, 'i');
    const results = pokemonListResponse.pokemon
      .filter((pokemon: any) => searchRegex.test(pokemon.name))
