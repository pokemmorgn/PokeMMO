...
export class InventorySystem {
  ...
  // Exporter les données du système pour la sauvegarde
  exportSystemData() {
    return {
      inventoryUI: this.inventoryUI.exportData(),
      lastUpdate: Date.now()
    };
  }

  // Importer les données lors du chargement
  importSystemData(data) {
    if (data.inventoryUI) {
      this.inventoryUI.importData(data.inventoryUI);
    }
  }

  // === MÉTHODES DE DEBUGGING ===

  // Ajouter des objets pour les tests (développement seulement)
  debugAddItem(itemId, quantity = 1) {
    if (this.gameRoom) {
      this.gameRoom.send("debugAddItem", {
        itemId: itemId,
        quantity: quantity
      });
    }
  ...
  // Afficher l'état complet de l'inventaire dans la console
  debugPrintInventory() {
    console.log("🎒 État de l'inventaire:", this.inventoryUI.inventoryData);
  }

  // Forcer la synchronisation avec le serveur
  forceSyncWithServer() {
    this.requestInventoryData();
    console.log("🎒 Synchronisation forcée avec le serveur");
  }
  ...
  destroy() {
    ...
    if (this.inventoryUI) {
      // L'UI se nettoie automatiquement via les événements
    }
  ...
  static getItemRarity(itemId) {
    // Déterminer la rareté d'un objet
    const rarityMap = {
  ...
  static getItemCategory(itemId) {
    // Catégoriser les objets pour l'affichage
    if (itemId.includes('ball')) return 'capture';
    if (itemId.includes('potion') || itemId.includes('heal')) return 'healing';
    if (itemId.includes('repel')) return 'utility';
    if (itemId.includes('key') || itemId.includes('card')) return 'key';
