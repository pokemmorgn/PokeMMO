...
export class BaseZoneScene extends Phaser.Scene {
  ...
  // Créer des objets de test dans la scène
  createTestItems() {
    console.log(`🧪 [${this.scene.key}] Création d'objets de test...`);
    ...
    // Créer plusieurs objets pour tester
    const testItems = [
  ...
  // Version avancée de création d'objet avec emoji
  createAdvancedWorldItem(itemId, x, y, emoji = '📦') {
    // Créer un conteneur pour l'objet
    const itemContainer = this.add.container(x, y);
    
    // Fond de l'objet
    const background = this.add.circle(0, 0, 12, 0x4a90e2);
    background.setStrokeStyle(2, 0xffffff);
    
    // Emoji de l'objet
    const itemEmoji = this.add.text(0, 0, emoji, {
      fontSize: '16px',
      align: 'center'
    }).setOrigin(0.5);
    ...
    itemContainer.on('pointerdown', () => {
      this.attemptPickupAdvancedItem(itemContainer);
    });
    ...
    console.log(`🎁 [${this.scene.key}] Objet avancé créé: ${itemId} à (${x}, ${y})`);
  }

  // Version améliorée du ramassage
  attemptPickupAdvancedItem(itemContainer) {
    const myPlayer = this.playerManager?.getMyPlayer();
    if (!myPlayer) return;
  ...
  // Effet de ramassage amélioré
  createAdvancedPickupEffect(x, y, itemId) {
    // Texte principal
    const mainText = this.add.text(x, y - 20, `+1 ${itemId}`, {
      fontSize: '14px',
      fontFamily: 'Arial',
      color: '#00ff00',
      stroke: '#000000',
