export class ItemEditorModule {
    ...
    async onTabActivated() {
        ...
        try {
            ...
            // Charger les statistiques pour les filtres
    ...
    setupEventListeners() {
        ...
        // DÉLÉGATION D'ÉVÉNEMENTS - Solution universelle
        document.addEventListener('change', (e) => {
            if (!e.target.tagName === 'SELECT') return;
            
            // Détecter les filtres de catégorie
            if (e.target.id.toLowerCase().includes('category') || 
                e.target.className.toLowerCase().includes('category') ||
                e.target.closest('[class*="category"]')) {
            ...
            // Détecter les filtres de génération
            if (e.target.id.toLowerCase().includes('generation') || 
                e.target.className.toLowerCase().includes('generation') ||
                e.target.closest('[class*="generation"]')) {
            ...
            // Détecter les filtres de rareté
            if (e.target.id.toLowerCase().includes('rarity') || 
                e.target.className.toLowerCase().includes('rarity') ||
                e.target.closest('[class*="rarity"]')) {
        ...
        document.addEventListener('input', (e) => {
            ...
            if (!e.target.placeholder || !e.target.placeholder.toLowerCase().includes('recherch')) return;
            
            clearTimeout(this.searchTimeout);
            this.searchTimeout = setTimeout(() => {
                console.log('🔍 [ItemEditor] Recherche:', e.target.value);
    ...
    async loadStats() {
        ...
        try {
            ...
            if (response.success) {
                ...
                console.log(`✅ [ItemEditor] Stats: ${this.stats.active} items actifs, ${Object.keys(this.stats.byCategory || {}).length} catégories`);
            } else {
    ...
    async loadItems() {
        ...
        try {
            ...
            while (true) {
                const response = await this.adminPanel.apiCall(`/items/list?page=${page}&limit=${limit}`);
    ...
    async performSearch() {
        try {
            ...
            this.applyFilters(); // Fallback sur filtrage local
        }
    }
