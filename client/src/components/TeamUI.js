...
// ✅ Version optimisée compatible avec le nouveau UIManager (SANS toggleModule)
...
export class TeamUI {
  ...
  /**
   * ✅ TOGGLE SIMPLE SANS UIManager.toggleModule
   */
  toggle() {
    if (this.isVisible) {
      this.hide();
...
console.log('✅ TeamUI adapté UIManager Professional chargé (SANS toggleModule)');
...
console.log('🔧 CORRECTION: Plus d\'erreur toggleModule - utilise show()/hide() directement');
  constructor(gameRoom) {
    this.gameRoom = gameRoom;
    this.isVisible = false;
    this.teamData = [];
